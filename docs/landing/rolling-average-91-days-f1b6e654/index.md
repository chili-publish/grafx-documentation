...
# Your 3-month rolling average

Your subscription measures render usage against your quota using a **3-month rolling average**.

In practice, that average is calculated over a **rolling window of 91 days** — not over three calendar months.
...
!!! Formula
    ...
    **Window** = the last 91 days, counted back from today

    **Rolling average** = total of past 91 days divided by 3
...
- **A single busy day stays in the calculation for 91 days.** A large campaign will keep lifting your average for roughly three months, then drop out of the window on its own.
...
This is why a temporary burst is usually fine. There is no hard limit, so you can always burst to much higher than your quota — and the spike is divided across the full 91-day window, which flattens its effect on the average considerably.
...
| `averageRendersPeriodDays` | The averaging period in days — `91` for your subscription |
