...
Smart Crop is the GraFx Genie capability that positions an image inside a frame automatically — keeping the important part of the picture in view no matter the frame's size or aspect ratio. One source image can become a square social post, a wide banner, and a tall print ad, each correctly framed, without manual cropping for every variant.

This page is the single reference for what Smart Crop is and how it works. The step-by-step guides for *doing* the work live in GraFx Media, GraFx Studio, and the platform settings — all linked below.

> **TODO (build session):** add a short hero image or the feature video here, matching the existing Smart Crop pages.

## How Smart Crop works, end to end
...
2. GraFx Genie analyses it and stores two pieces of metadata on the asset: a **Subject Area** and a **Point of Interest**.
3. The asset is placed into an image frame in a GraFx Studio Smart Template.
4. The frame's fit mode is set to **Smart Crop**.
5. Genie positions the image so the subject stays in view, respecting the frame's size and aspect ratio.
...
| **Fit mode** | How an image sits in its frame: Fit, Fill, Smart Crop, or Manual. Smart Crop is one of these modes. |

## How GraFx Genie detects the subject

When an image is uploaded (or reprocessed), GraFx Genie Vision examines it and sets the Default Subject Area and Point of Interest. Genie recognises people and a range of contextual objects, and places the Point of Interest on the most relevant feature — for a person, that means detecting the eyes.

!!! info "Applies to new or reprocessed assets"
    Improvements to Genie's detection apply to newly uploaded assets, or to assets that are reprocessed. Existing assets keep their stored Subject Area and POI until they are reprocessed.

> **TODO / [CONFIRM] (build session):**
>
> - List what Genie currently detects (people, contextual objects such as buildings) and confirm that logo detection is no longer performed — so examples here must not imply automatic logo detection. Source: GraFx Genie Vision release note, Jul 2025; confirm current state with `chili-docs`.
> - Explain Smart Crop algorithm **versioning**: how versions work and how an asset/template picks up a newer version. **[CONFIRM]**

## Subject types — one asset, several crop intents

A single image can hold more than one Subject Area and Point of Interest, grouped under a **subject type** — for example *person* or *product*. The same photo can then crop to the person for a social ad and to the product for a catalogue page, without duplicating the asset or rebuilding the frame.
...
- Leave some space around the main subject in the original image, so Genie has room to reposition without creating whitespace or scaling the image up.
...
Smart Crop handles most cases, but some images — lifestyle photography especially — need a human touch:

- **Subject Position** lets you steer where the subject lands in a specific frame (for example, to the left when text sits on the right). See [Use Smart Crop on an image frame](/GraFx-Studio/guides/smart-crop/).
...
> **TODO / [CONFIRM] (build session):** supported file types, minimum resolution, where Genie runs, and any credit/billing implications of Genie processing. Confirm with `chili-docs` / an SME before publishing.
