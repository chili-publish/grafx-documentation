<!--
  GraFx Genie hub — repeatable structure (follow this for every Genie capability)

  This section is a LANDING HUB. Keep this index page minimal: one short intro plus
  one line per capability linking to its own page below.

  List actual Genie CAPABILITIES here — not the places they live. (For example, list
  "Product Image Creator" and "Product Image Composer", not "GraFx Labs", which is only
  where those features are hosted.)

  Every GraFx Genie capability gets its own page under GraFx-Genie/concepts/<capability>/
  that follows the same three-part pattern:
    1. What it is        — 1–2 plain-language sentences.
    2. How GraFx Genie helps — the AI angle: what Genie does for the user here.
    3. Where to find it  — links OUT to where the feature actually lives (which may be one
       place, several, or GraFx Labs). E.g. Smart Crop lives in GraFx Media, GraFx Studio,
       and platform settings; the image features live in GraFx Labs.
