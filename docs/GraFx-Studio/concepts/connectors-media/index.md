...
![Diagram of GraFx Studio linking through one Media Connector to GraFx Media, a DAM and other providers](connector2.png){.screenshot-full}
...
![The Media panel listing two connectors, GraFx Media and Acquia DAM, beside a phone ad template](damconnector01.png){.screenshot-full}
...
![Search results in the Media panel, showing every asset the connector returns for one query](damconnector02.png){.screenshot-full}
...
![Connector settings for an image variable, with the search query linked to a variable instead of a fixed value](damconnector03.png){.screenshot-full}

In the end-user interface, the end-user will see the selected assets. In this example, a category of smartphones:


![The end-user Select image panel, narrowed down to smartphone assets from the DAM](damconnector04.png){.screenshot-full}
...
![The Metadata mapping dialog, linking metadata keys such as brandname and productCPU to variables](damconnector05.png){.screenshot-full}

![Product specifications on the canvas, filled from the metadata of the asset the end user picked](damconnector06.png){.screenshot-full}
