...
**PDF assets are not flattened.** The placed PDF keeps its vector content in the output. It is not rasterized, not converted to an image, and not re-compressed. Text stays text, vector artwork stays vector, and spot colors stay separate inks.
...
| Text | Stays text, using the embedded font. It is not converted to outlines or rasterized |
