...
!!! info "Good to know"
    ...
    - **Descriptions of media, colors and text styles are shared across all themes.** A description explains what an element is for, so you set it on the default theme; in other themes it is read-only. See [Elements](/GraFx-Brand-Kits/concepts/elements/#descriptions).
