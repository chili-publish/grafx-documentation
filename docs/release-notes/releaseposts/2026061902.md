...
Smart Crop is the GraFx Studio fill mode that automatically zooms and positions an image inside a frame so the most important part stays in view. This release is a major upgrade:

- **A more predictable algorithm.** The new version needs far less trial-and-error. It always fills the frame with no empty edges, always keeps the Point of Interest inside the Subject Position, and fits as much of the Subject Area as it can — only cropping in as a last resort.
- **Subject alignment.** A new setting nudges the subject toward any side of the Subject Position instead of always centering it, so a subject can hug a layout edge and stay there across different aspect ratios.
...
- **Five live previews.** The asset details panel now shows five preview aspect ratios instead of three, in both GraFx Media and the Studio workspace, plus a clearer view while you position a subject.
