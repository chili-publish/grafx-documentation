---
draft: false
date: 2026-08-31T14:36:10
categories:
  - Releases
---

# GraFx Studio: Figma plugin (experimental)

![GraFx Studio icon](/assets/icon-GraFx-Studio.svg){.rn_icon}

![The GraFx Studio Exporter panel open in Figma, showing the selected page and frame, a clean preflight result, and the Export button next to the design being exported](/release-notes/releasenotesassets/figma-exporter.png){.screenshot-full}

The **GraFx Studio Exporter for Figma** turns a Figma frame into a GraFx Studio template, joining the existing plugins for Adobe® InDesign® and Adobe® Photoshop®. It runs on Windows and macOS.

## ✨ New & Improved

- Text, text styles, and color styles convert to editable GraFx Studio elements
- Shapes, images, gradients, drop shadows, opacity, and blend modes are supported
- Preflight reports what GraFx Studio cannot reproduce exactly, before you export

## ℹ️ Experimental release

Version **0.8.0 Experimental**. It is not yet available from the Figma Community or the [plugin downloads](/GraFx-Studio/convert/downloads/) page — contact your CHILI publish contact for access. As with anything marked [experimental](/release-notes/experimental/), supported features and behavior can still change between versions.

## More info

Read more on the [Figma plugin](/GraFx-Studio/convert/Figma/) page
