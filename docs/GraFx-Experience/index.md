# GraFx Experience

GraFx Experience is the end-user portal where brand users can browse, personalize, and produce on-brand content using Smart Templates built in GraFx Studio.

The portal is set up and managed by CHILI publish. Customers access it — they do not build it.

The connection between the portal and GraFx Studio is provided by the **CHILI GraFx extension**.

## In this section

- [GraFx Experience](concepts/grafx-experience/) — What the portal is and how it works
- [CHILI GraFx extension](concepts/chili-grafx-extension/) — How Smart Templates are made available in the portal
- [Configure a Smart Template](guides/configure-smart-template/) — How to link and configure a GraFx Studio template for use in GraFx Experience